...
export class MomentsController {
  ...
  @ApiOperation({
    ...
    description:
      '이미지 파일은 "이미지 URL 생성 API"로 변환 후 URL 을 입력해주시면 됩니다.',
  })
  ...
  async createMoment(
    @GetUser() user,
    @Body(ValidationPipe) body: CreateMomentDto,
    @Res() res,
  ) {
    const responseData = await this.momentsService.createMoment(
      user.userIdx,
      body,
    );
    return res.json(
  ...
  @Get('/my-history')
  @UseGuards(JwtAuthGuard)
  async getMyMomentHistory(
    @GetUser() user,
    @Query(ValidationPipe) query: GetHistoryRequest,
    @Res() res,
  ) {
    const responseData = await this.momentsService.getMyMoments(
      user.userIdx,
      query,
    );
    return res.json(
  ...
  async getMoments(
    ...
    const responseData = await this.momentsService.getMomentsByPin(
      user.userIdx,
      pinIdx,
      query,
    );
  ...
  async deleteMoment(
    @GetUser() user,
    @Param('momentIdx') momentIdx: number,
    @Res() res,
  ) {
